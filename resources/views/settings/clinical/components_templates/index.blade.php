...
@section('title', ' Components Templates')
                    ...
                    <div class="offcanvas-body my-auto mx-0 flex-grow-0">
                        <form id="add_data_form" method="POST">
                            <div class="mb-3">
                                ...
                                <input name="name" type="text" class="form-control" placeholder="In English"/>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Template (English) </label>
                                <textarea name="template_en" rows="5" class="form-control" placeholder="In English"></textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Template (বাংলা) </label>
                                <textarea name="template_bn" rows="5" class="form-control text-bn" placeholder="বাংলায়"></textarea>
                            </div>
                            
                            <input name="template_type" value="{{ $template_type }}" type="hidden" />
    ...
    <script>
        ...
        $(function(){
            data_table = data_table.DataTable({
                ajax: "{{ route('components_templates', $template_type) }}",
                processing:true,
                serverSide:true,
                columns: [
                            ...
                            return row.created_by == {{ auth()->id() }} ? `
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="javascript:void(0);" onclick="showData(${row.id})"><i class="ti ti-eye me-1"></i> View</a>
        ...
        function editData(id) {
            $.ajax({
                url: `{{ route('components_templates.find', ':id' ) }}`.replace(':id', id),
                type: "GET",
                success: function ( res ) {
                    if (res.success) {
                        ...
                        edit_data_form.find("textarea[name='template_en']").val(res.data.template_en);
                        edit_data_form.find("textarea[name='template_bn']").val(res.data.template_bn);
        ...
        add_data_form.on('submit', function(e) {
            if (!e.isDefaultPrevented()) {
                $.ajax({
                    url: `{{ route('components_templates.store') }}`,
                    type: "POST",
        ...
        edit_data_form.on('submit', function(e) {
            if (!e.isDefaultPrevented()) {
                ...
                $.ajax({
                    url: `{{ route('components_templates.update', ':id') }}`.replace(':id', id),
                    type: "POST",
        ...
        function changeStatus(id) {
            Swal.fire({
                ...
                if(t.value){
                    $.ajax({
                        url: `{{route('components_templates.change_status', ':id')}}`.replace(':id', id),
                        type: 'GET',
                        success: function(data) {
                            if (data.success === true) {
                                data_table.ajax.reload();
        ...
        function deleteData(id) {
            Swal.fire({
                ...
                if(t.value){
                    $.ajax({
                        url: `{{route('components_templates.destroy', ':id')}}`.replace(':id', id),
                        type: 'POST',
                        data: {'_method': 'DELETE',},
                        success: function(data) {
                            if (data.success === true) {
