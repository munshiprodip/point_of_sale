...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="card mb-4">
                <h5 class="card-header">New Requisition</h5>
                <div class="card-body">
...
@section('script')
    <script>
        ...
        $(function(){
            select_product_form.on('submit', function(e) {
                ...
                if (!e.isDefaultPrevented()) {
                    $.ajax({
                        url: `{{ route('requisitions.add_to_cart') }}`,
                        type: "POST",
                        data: formData,
                        contentType: false,
                        processData: false,
            ...
            cart_data_table = cart_data_table.DataTable({
                ajax: "{{ route('requisitions.create') }}",
                searching: false, 
                ordering: false, 
                processing:true,
                serverSide:true,
        ...
        function deletFromCart(id) {
            $.ajax({
                url: `{{route('requisitions.delete_from_cart', ':id')}}`.replace(':id', id),
                type: 'POST',
                data: {'_method': 'DELETE',},
                success: function(data) {
        ...
        function prepareRequisition() {
            Swal.fire({
                title: "Are you sure?",
                text: "You want to prepare requisition with selected items?",
                icon: "warning",
                showCancelButton: !0,
                ...
                if(t.value){
                    $.ajax({
                        url: `{{route('requisitions.prepare')}}`,
                        type: 'POST',
                        data: {},
                        success: function(data) {
                            if (data.success === true) {
